...
// Worst Case: each trie linked to 230 (Capital letters not stored) other trie = effectively Constant since Ceil(Log(230)) = 8
// Hence, can be said to be dependent on the length of key since
// Example: (Store alt)
...
public:
    ...
    ~Graph();
    Genre<Movie*>* add_category(string); // O(Logn)
    Genre<Movie*>* find_category(string); //O(Logn)
